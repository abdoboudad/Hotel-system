...
<div class="card p-3">
    <form action="{{ route('user.update',$user->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="email" class="form-control" value="{{ $user->email}}" name="email" id="useremail" placeholder="{{ __('custom.enter_email') }}" required="">
            @error('email')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control " value="{{ $user->name }}" name="name" id="username" placeholder="{{ __('custom.enter_username') }}" required="">
            @error('name')
                <div class="invalid-feedback">
                    {{ __('custom.enter_email_message') }}
                </div>
        ...
        <div class="mb-3">
            ...
            <div class="position-relative auth-pass-inputgroup">
                <input type="password" class="form-control password-input pe-2" onpaste="return false" placeholder="{{ __('custom.enter_password') }}" id="password-input" name="password" >
            </div>
        </div>

        <div class="mb-3">
            <label for="username" class="form-label">{{ __('custom.password_confirmation') }} <span class="text-danger">*</span></label>
            <input type="password" class="form-control pe-2" name="password_confirmation" id="username" placeholder="{{ __('custom.confirm_password') }}">
